 ...
 *
 * A food is standard unless every recipe using it flags it as an extra. Four
 * are claimed both ways by different recipes — whey, garlic, chocolate chips
 * and cocoa — and the shopping list already resolves those in favour of the
 * storehouse, so this agrees with it.
 * ------------------------------------------------------------------------- */
...
out.forEach((r) => (r.ingp || []).forEach((it) => {
  ...
  if (extraOnly[it.k] === undefined) extraOnly[it.k] = true;
  if (!it.x) extraOnly[it.k] = false;
}));
const pantry = {};
Object.keys(CATS).sort().forEach((k) => {
  pantry[k] = { l: (shop[k] && shop[k].l) || k.replace(/_/g, ' '), c: CATS[k], s: !extraOnly[k] };
});
const usedNotCategorised = Object.keys(extraOnly).filter((k) => !CATS[k]);
